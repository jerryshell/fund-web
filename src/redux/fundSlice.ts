...
export const fundSlice = createSlice({
    ...
    reducers: {
        setFundData: (state, action: PayloadAction<FundData[]>) => {
            state = action.payload;
        },
        addFund: (state, action: PayloadAction<{ fund: FundData }>) => {
            state.push(action.payload.fund);
        },
        removeFundByCode: (state, action: PayloadAction<{ code: string }>) => {
            state.splice(state.findIndex(fund => fund.code === action.payload.code), 1);
        },
        setJerryIndexByCode: (state, action: PayloadAction<{ code: string, jerryIndex: number }>) => {
            const fund = state.find(fund => fund.code === action.payload.code);
            if (fund) {
...
export const fundSliceActions = fundSlice.actions
